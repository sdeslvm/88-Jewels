...
protocol JewelsLoadStateRepresentable {
    var type: JewelsLoadState.StateType { get }
...
struct JewelsLoadState: Equatable, JewelsLoadStateRepresentable {
    ...
    static func idle() -> JewelsLoadState {
        JewelsLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> JewelsLoadState {
        JewelsLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> JewelsLoadState {
        JewelsLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> JewelsLoadState {
        JewelsLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> JewelsLoadState {
        JewelsLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: JewelsLoadState) -> Bool {
    ...
    static func == (lhs: JewelsLoadState, rhs: JewelsLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension JewelsLoadState: CustomStringConvertible {
